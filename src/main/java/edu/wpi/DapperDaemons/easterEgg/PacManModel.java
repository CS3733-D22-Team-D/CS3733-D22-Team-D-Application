/**
 * @author Jessie Baskauf and Ellie Mamantov The Model stores information about the game state,
 *     including the underlying grid of CellValues (as loaded from the text file), various boolean
 *     indicators about game state, level, score, and the movement of PacMan and ghosts.
 */
...
public class PacManModel {
  ...
  /**
   * Configure the grid CellValues based on the txt file and place PacMan and ghosts at their
   * starting locations. "W" indicates a wall, "E" indicates an empty square, "B" indicates a big
   * dot, "S" indicates a small dot, "1" or "2" indicates the ghosts home, and "P" indicates
  ...
  /**
   * Move PacMan based on the direction indicated by the user (based on keyboard input from the
   * Controller)
   *
  ...
  public void movePacman(Direction direction) {
    ...
    if (direction.equals(lastDirection)) {
      ...
      if (grid[(int) potentialPacmanLocation.getX()][(int) potentialPacmanLocation.getY()]
          == CellValue.WALL) {
    ...
    // if most recent direction input is not the same as previous input, check for walls and corners
    // before going in a new direction
    else {
      // if PacMan would hit a wall with the new direction input, check to make sure he would not
      // hit a different wall if continuing in his previous direction
      if (grid[(int) potentialPacmanLocation.getX()][(int) potentialPacmanLocation.getY()]
          == CellValue.WALL) {
  ...
  /**
   * Move a ghost to follow PacMan if he is in the same row or column, or move away from PacMan if
   * in ghostEatingMode, otherwise move randomly when it hits a wall.
   *
  ...
  public Point2D[] moveAGhost(Point2D velocity, Point2D location) {
    ...
    if (!ghostEatingMode) {
      ...
      if (location.getY() == pacmanLocation.getY()) {
        ...
        while (grid[(int) potentialLocation.getX()][(int) potentialLocation.getY()]
            == CellValue.WALL) {
      ...
      // move in a consistent random direction until it hits a wall, then choose a new random
      // direction
      else {
        Point2D potentialLocation = location.add(velocity);
        potentialLocation = setGoingOffscreenNewLocation(potentialLocation);
    ...
    // if the ghost is in the same row or column as Pacman and in ghostEatingMode, go in the
    // opposite direction
  ...
  /**
   * Updates the model to reflect the movement of PacMan and the ghosts and the change in state of
   * any objects eaten during the course of these movements. Switches game state to or from
   * ghost-eating mode.
   *
   * @param direction the most recently inputted direction for PacMan to move in
   */
  public void step(Direction direction) {
    ...
    CellValue pacmanLocationCellValue =
        grid[(int) pacmanLocation.getX()][(int) pacmanLocation.getY()];
    if (pacmanLocationCellValue == CellValue.SMALLDOT) {
      grid[(int) pacmanLocation.getX()][(int) pacmanLocation.getY()] = CellValue.EMPTY;
      dotCount--;
      score += 10;
    }
    // if PacMan is on a big dot, delete big dot and change game state to ghost-eating mode and
    ...
    // move ghosts and checks again if ghosts or PacMan are eaten (repeating these checks helps
    // account for even/odd numbers of squares between ghosts and PacMan)
    this.moveGhosts();
    if (ghostEatingMode) {
      if (pacmanLocation.equals(ghost1Location)) {
        sendGhost1Home();
  ...
  /**
   * Connects each direction to Point2D velocity vectors (Left = (-1,0), Right = (1,0), Up = (0,-1),
   * Down = (0,1))
   *
   * @param direction
