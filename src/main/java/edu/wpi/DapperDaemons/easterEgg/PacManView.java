/**
 * @author Jessie Baskauf and Ellie Mamantov Encorporates the various Views of the application that
 *     reference different parts of the Model, including the main game board, the score label, the
 *     level label, and the Game Over label.
 */
  ...
  public void update(PacManModel model) {
    ...
    for (int row = 0; row < this.rowCount; row++) {
      for (int column = 0; column < this.columnCount; column++) {
        ...
        if (row == model.getPacmanLocation().getX()
            && column == model.getPacmanLocation().getY()
        ...
        } else if (row == model.getPacmanLocation().getX()
            && column == model.getPacmanLocation().getY()
            && PacManModel.getLastDirection() == PacManModel.Direction.LEFT) {
          this.cellViews[row][column].setImage(this.pacmanLeftImage);
        } else if (row == model.getPacmanLocation().getX()
            && column == model.getPacmanLocation().getY()
            && PacManModel.getLastDirection() == PacManModel.Direction.UP) {
          this.cellViews[row][column].setImage(this.pacmanUpImage);
        } else if (row == model.getPacmanLocation().getX()
            && column == model.getPacmanLocation().getY()
            && PacManModel.getLastDirection() == PacManModel.Direction.DOWN) {
          this.cellViews[row][column].setImage(this.pacmanDownImage);
        }
        // make ghosts "blink" towards the end of ghostEatingMode (display regular ghost images on
        // alternating updates of the counter)
        if (PacManModel.isGhostEatingMode()
            && (Controller.getGhostEatingModeCounter() == 6
