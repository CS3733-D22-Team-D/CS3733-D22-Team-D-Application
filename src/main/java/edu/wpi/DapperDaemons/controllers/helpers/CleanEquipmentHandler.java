...
public class CleanEquipmentHandler {
    ...
    public void checkAndUpdateCleanRequests() {
        List<MedicalEquipment> cleanEquipment = new ArrayList<>(medicalEquipmentDAO.filter(5, "CLEAN").values());
        List<MedicalEquipment> uncleanEquipment = new ArrayList<>(medicalEquipmentDAO.filter(5, "UNCLEAN").values());
        List<MedicalEquipment> inProgressEquipment = new ArrayList<>(medicalEquipmentDAO.filter(5, "INPROGRESS").values());

        for (MedicalEquipment.EquipmentType type : MedicalEquipment.EquipmentType.values()) {
            List<MedicalEquipment> uncleanEqOfType = new ArrayList<>(medicalEquipmentDAO.filter(uncleanEquipment, 3, type.name()).values());
            List<MedicalEquipment> cleanEqOfType = new ArrayList<>(medicalEquipmentDAO.filter(cleanEquipment, 3, type.name()).values());
            List<MedicalEquipment> inProgressEqOfType = new ArrayList<>(medicalEquipmentDAO.filter(inProgressEquipment, 3, type.name()).values());
